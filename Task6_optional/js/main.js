...
expensesBtn.addEventListener('click', function(){
    ...
    for (let i = 0; i < expensesItem.length; i++){
        ...
        if ( (typeof(item)) === 'string' && item != null && price != null
            && item != '' && price != '' && item.length < 50 && (/[0-9]/.test(price))) {
            console.log("done");    
...
optionalExpensesBtn.addEventListener('click', function(){
    for (let k = 0; k < optionalExpensesItem.length; k++) {
        let opt = optionalExpensesItem[k].value;
        if (/[а-яА-яЁё]/.test(opt)) {
            appData.optionalExpenses[k] = opt;
            optionalExpensesValue.textContent += appData.optionalExpenses[k] + ' ';
        } else {
            console.log('Латиница не приветствуется! Используйте кириллицу!');
        }
    }
});
